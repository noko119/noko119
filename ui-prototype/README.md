...
- **工业绘图（P2）**
  - **插入竖曲线**：选中转角 → 凸/凹弧 + **正式 Rmin**（张力/速度/下垂度）
  - **插入水平弯**：XY 平面弧 + Z 插值
  - **重算包角**：邻段几何写 φ + **欧拉不打滑**摘要
  - **D/2 偏移**：滚筒节点按直径角平分线修正中心线
- **剖面确认**
  - **配对航段** / **强制间距跟随**：承载↔回程配对；回程硬锁定间距重算
  - **Finalize**：闭环/主驱/D/包角/正式竖曲线 Rmin/不打滑
...
- 引擎：`calc/dtii-engine.js`（**1:1 / 2:1 / 1:2** 双驱配比 + **电机选型上靠**）
- 锚定算例：`calc/gc01-case.js`（**v1 唯一黄金算例 GC-01**）
- 系数面板：`calc/default-coeffs.js` → `calc.html` 可改 f/C/η/μ/v/q*/FS*、工况预设；**冻结**→`sessionStorage pidm.coeff.freeze.v0`
...
- 大厂缺口补齐（本轮）：
  ...
  - **承载↔回程配对航段**
- 正式化续作：
  - **竖曲线正式 Rmin**（张力 + 速度 + 下垂度分解）
  - **欧拉不打滑**校核与包角摘要
  - **DTⅡ 十二大项**自动分类（`dtii-flight-dict.js`）+ 强制间距跟随
  - **水平弯**插入（XY）
  - **系数/工况面板** + 冻结快照；引擎 **电机选型**步骤

### 本轮续完善
- **P2P→路径设计荷载桥接**：计算页运行后写入 session，路径 Finalize/竖曲线/不打滑/水平弯共用
- **水平弯 Rmin**：max(v²/a_lat, 速度地板, 带宽地板)
- **path-schema** 回程大项统一为 `retH`/`retI`
- **胶带 ST 档选型**步骤 + 计算摘要展示电机/胶带
