...
function exportSwJson() {
  ...
  let pkg;
  try {
    pkg = exportConeMoldSwPackage(last);
  } catch (err) {
    $("status").textContent = `导出异常：${err?.message || err}`;
    $("status").className = "status error";
    return;
  }
  if (!pkg.ok) {
  ...
  const name = `cone-mold-v0-${Date.now()}.json`;
  // 先更新状态，避免下载被拦时看起来像“没反应”
  ...
  try {
    ...
    // 下载受限时回退到复制剪贴板，网页方案本身不受影响
    try {
      navigator.clipboard.writeText(text);
      $("status").textContent = `下载受限，已复制 JSON（${pkg.schema}，${name}）`;
    } catch {
      $("status").textContent = `已生成 JSON 但浏览器拦截下载：${err?.message || err}`;
    }
    $("status").className = "status ok";
  }
}
