...
def main():
    ...
    for row, lab, val, note in labels:
        ...
        if row in (5, 6, 7, 8, 9, 10, 11, 12, 13, 15, 17, 18, 19):
    ...
    wsI["A21"] = "手选（可改下拉；选「用推荐」则跟推荐走）"
    sty(wsI["A21"], H)
    wsI.merge_cells("A21:C21")
    hand = [
        (22, "手选丝杠", "（用推荐）"),
        (23, "手选伺服", "（用推荐）"),
        (24, "手选步进", "（用推荐）"),
    ...
    calcs = [
        ...
        (20, "齿条转速", "=输入!B9*1000/(PI()*输入!B18*输入!B19)"),
        (21, "齿条工作转矩", "=B6*(输入!B18*输入!B19)/2000/输入!B17"),
        (22, "齿条启动转矩", "=B15/MAX(B14,0.01)*B21"),
        (23, "有效齿条模数", '=IF(输入!B25="（用推荐）",输入!B18,VALUE(输入!B25))'),
        (24, "有效齿条齿数", '=IF(输入!B26="（用推荐）",输入!B19,VALUE(输入!B26))'),
        (25, "手选后齿条转速", "=输入!B9*1000/(PI()*B23*B24)"),
        (26, "手选后齿条转矩", "=B6*(B23*B24)/2000/输入!B17"),
        (27, "有效丝杠", '=IF(输入!B22="（用推荐）",B12,输入!B22)'),
        ...
        (32, "齿条推荐伺服", '=IFERROR(INDEX(目录电机!B2:B7,MATCH(1,目录电机!O2:O7,0)),"无满足项")'),
        (33, "齿条推荐步进", '=IFERROR(INDEX(目录电机!B8:B12,MATCH(1,目录电机!O8:O12,0)),"无满足项")'),
        (34, "有效伺服", '=IF(输入!B23="（用推荐）",B30,输入!B23)'),
        (35, "有效步进", '=IF(输入!B24="（用推荐）",B31,输入!B24)'),
    ...
    rec = [
        ...
        (12, "推荐步进（丝杠方案）", "=计算!B31"),
        (13, "丝杠机械功率 kW", "=计算!B36"),
        (15, "推荐齿条", '=TEXT(输入!B18,"0.0")&"模数 × "&TEXT(输入!B19,"0")&"齿 "&输入!B16'),
        (16, "齿条转速 r/min", "=计算!B20"),
        (17, "齿条转矩 N·m", "=计算!B21"),
        (18, "推荐伺服（齿条方案）", "=计算!B32"),
        (19, "推荐步进（齿条方案）", "=计算!B33"),
        (20, "齿条机械功率 kW", "=计算!B37"),
    ...
    rows_d = [
        (4, "效率", "=输入!B15", "=输入!B17"),
        (5, "电机转速 r/min", "=计算!B13", "=计算!B20"),
        (6, "需要转矩 N·m", "=计算!B14", "=计算!B21"),
        (7, "机械功率 kW", "=计算!B36", "=计算!B37"),
        (8, "推荐伺服", "=计算!B30", "=计算!B32"),
        (9, "推荐步进", "=计算!B31", "=计算!B33"),
        (10, "支承", '=计算!B16&" + "&计算!B17', "齿轮轴轴承（不是BK座）"),
        (11, "行程适应性", '=IF(输入!B10>4000,"长行程丝杠易垂","短中行程合适")', '=IF(输入!B10>4000,"长行程更合适","短行程也能用")'),
        (12, "效率谁高", "", '=IF(输入!B17>输入!B15,"齿条效率更高","丝杠效率更高或相当")'),
    ]
    ...
    for i, h in enumerate(["项目", "手选丝杠方案", "手选齿条方案"], 1):
        sty(wsD.cell(15, i, h), H, YELLOW)
    hand_rows = [
        (16, "型号/参数", "=计算!B27", '=TEXT(计算!B23,"0.0")&"×"&TEXT(计算!B24,"0")&" "&输入!B16'),
        (17, "转速", "=计算!B28", "=计算!B25"),
        (18, "转矩", "=计算!B29", "=计算!B26"),
        (19, "伺服", "=计算!B34", "=计算!B34"),
        (20, "步进", "=计算!B35", "=计算!B35"),
    ...
    wsD["A22"] = "手选电机的安全系数见「目录电机」里对应行的运行SF；低于目标安全系数不要当正式方案。"
    sty(wsD["A22"], Font(name="微软雅黑", size=10), wrap=True)
    wsD.merge_cells("A22:C22")
    colw(wsD, [22, 42, 42])
    ...
    wsE["A3"] = "机械工程师"
    sty(wsE["A3"], H, BLUE)
    wsE["A4"] = '=CONCATENATE("工作推力 ",TEXT(计算!B6,"0")," N，总质量 ",TEXT(计算!B2,"0.0")," kg。丝杠推荐 ",计算!B12,"，座 ",计算!B16,"+",计算!B17,"（角接触 ",计算!B18,"；深沟球 ",计算!B19,"）。齿条按模数",TEXT(输入!B18,"0.0")," 齿数",TEXT(输入!B19,"0"),"。BK只配丝杠，不要装到齿条上。")'
    ...
    lines = [
        ...
        "5. 丝杠和齿轮齿条用同一推力对比效率和电机。BK/BF 只属于丝杠。",
        "6. 会审页是工艺、机械、计算机三句意见，随计算结果变。",
        "7. 用 Excel 或 WPS 打开。若公式显示为文字，检查单元格格式为「常规」并允许计算。",
        "8. 目录里 3210/3220 标现货=0，自动推荐不会选它们，手选仍可选（常无货）。",
        "公式：F = mg·sinθ + μmg·cosθ + ma + F外；丝杠 T=F·P/(2πη)；齿条 T=F·(mz/2000)/η；n丝杠=v×1000/P；n齿条=v×1000/(π m z)。",
    ]
